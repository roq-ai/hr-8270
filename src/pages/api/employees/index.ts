...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createEmployee() {
    ...
    const body = { ...req.body };
    if (body?.engagement_tool?.length > 0) {
      const create_engagement_tool = body.engagement_tool;
      body.engagement_tool = {
        create: create_engagement_tool,
      };
    } else {
      delete body.engagement_tool;
    }
